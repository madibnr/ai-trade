...
def build_prompt_context(df: pd.DataFrame, tick: Any, active_positions: list) -> str:
    """
...
def get_ai_decision(df: pd.DataFrame, tick: Any, active_positions: list) -> Dict[str, Any]:
    """
    ...
    context_prompt = build_prompt_context(df, tick, active_positions)
...
Tugas Anda adalah memberikan keputusan trading instan dalam format raw JSON.
...
1. Jangan menunggu konfirmasi tren sempurna atau persilangan (crossover) EMA yang lebar. Cukup deteksi dorongan momentum pada 1-2 candle terakhir, kemiringan EMA 9 terhadap EMA 21, dan bias RSI (di atas 50 = dorongan BUY, di bawah 50 = dorongan SELL).
2. KURANGI STATUS HOLD: Selalu berikan rekomendasi "BUY" atau "SELL" selama ada bias arah mikro sekecil apa pun dari candle terbaru. Hanya keluarkan "HOLD" jika spread sedang melonjak ekstrem di atas rata-rata atau pasar benar-benar stagnan tanpa pergerakan.
3. TARGET PROFIT TIPIS (Quick Out): Jarak Take Profit (TP) wajib diatur sangat dekat antara 0.8x hingga 1.2x ATR. (Contoh untuk XAUUSD berkisar antara $0.80 - $1.50; EURUSD antara 4 - 8 pips).
4. STOP LOSS KETAT: Jarak Stop Loss (SL) disiplin diatur antara 1.0x hingga 1.2x ATR. 
...
CONTOH FORMAT OUTPUT:
{
  ...
  "sl_price": 2750.00,
  "tp_price": 2752.50,
  "reason": "Momentum bullish agresif dari candle terakhir, RSI 55, target scalping 1x ATR."
}"""
